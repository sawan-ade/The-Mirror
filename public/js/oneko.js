...
(function oneko() {
  ...
  let nekoPosX = Math.max(100, Math.floor(window.innerWidth / 2));
  let nekoPosY = Math.max(100, Math.floor(window.innerHeight / 2));
  ...
  function init() {
    ...
    nekoEl.style.zIndex = '2147483647'; // Always visible on top
    nekoEl.style.backgroundImage = `url("${nekoSpriteB64}")`;
    nekoEl.style.filter = 'drop-shadow(0 2px 8px rgba(0,0,0,0.35))';
    nekoEl.style.transition = 'opacity 0.2s';
    nekoEl.style.display = 'block';

    document.body.appendChild(nekoEl);

    // Track cursor movements across both window and document
    const trackMouse = function (event) {
  ...
  window.toggleOneko = function (show) {
    if (show === undefined) {
      const isVisible = nekoEl.style.display !== 'none';
      nekoEl.style.display = isVisible ? 'none' : 'block';
    } else {
